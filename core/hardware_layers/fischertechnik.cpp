...
void updateBuffers()
{
	...
	for (i=0; i<8; i++)
	{
		sendBytes[1] = sendBytes[1] | (CoilsBuffer0[i] << i); //write each bit
	}
	for (i=8; i<16; i++)
	{
		sendBytes[2] = sendBytes[2] | (CoilsBuffer0[i] << (i-8)); //write each bit
	}
	...
	for (i=0; i<8; i++)
	{
		DiscreteInputBuffer0[i] = (recvBytes[0] >> i) & 0x01;
		//printf("%d\t", DiscreteInputBuffer0[i]);
	}
	for (i=8; i<16; i++)
	{
		DiscreteInputBuffer0[i] = (recvBytes[1] >> (i-8)) & 0x01;
		//printf("%d\t", DiscreteInputBuffer0[i]);
	}
	//printf("\n");
	pthread_mutex_unlock(&bufferLock);
