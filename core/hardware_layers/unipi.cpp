...
void updateBuffers()
{
	...
	for (int i = 0; i < MAX_INPUT; i++)
	{
		DiscreteInputBuffer0[i] = !digitalRead(inputPinMask[i]); //printf("[IO%d]: %d | ", i, !digitalRead(inputPinMask[i]));
	}

	//printf("\nDigital Outputs:\n");
	for (int i = 0; i < MAX_OUTPUT; i++)
	{
		digitalWrite(DOUT_PINBASE + i, CoilsBuffer0[i]); //printf("[IO%d]: %d | ", i, digitalRead(DOUT_PINBASE + i));
	}

	//printf("\nAnalog Inputs:");
	for (int i = 0; i < 2; i++)
	{
		AnalogInputBuffer0[i] = mcp_adcRead(i); //printf("[AI%d]: %d | ", i, mcp_adcRead(i));
	}
	//printf("\n");

	pwmWrite(ANALOG_OUT_PIN, AnalogOutputBuffer0[0]);
